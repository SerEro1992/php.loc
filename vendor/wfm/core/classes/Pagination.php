  ...
  public function getHtml(): string
  {
    ...
    if ($this->current_page > 1) {
      $back = '<li class="page-item">
                    <a class="page-link" href="' . $this->getLink($this->current_page - 1) . '">Previous
                    </a>
              </li>';
    }
    ...
    if ($this->current_page < $this->count_pages) {
      $forward = '<li class="page-item">
                    <a class="page-link" href="' . $this->getLink($this->current_page + 1) . '">Next
                    </a>
              </li>';
    }

    if ($this->current_page > $this->mid_size + 1) {
      $start_page = '<li class="page-item">
                    <a class="page-link" href="' . $this->getLink(1) . '">&laquo;
                    </a>
              </li>';
    }

    if ($this->current_page < $this->count_pages - $this->mid_size) {
      $end_page = '<li class="page-item">
                    <a class="page-link" href="' . $this->getLink($this->count_pages) . '">&raquo;
                    </a>
              </li>';
    }
    ...
    for ($i = $this->mid_size; $i > 0; $i--) {
      if ($this->current_page - $i > 0) {
        $pages_left .= '<li class="page-item">
                    <a class="page-link" href="' . $this->getLink($this->current_page - $i) . '">
                    ' . ($this->current_page - $i) . '
                    </a>
              </li>';
      }
    ...
    for ($i = 1; $i <= $this->mid_size; $i++) {
      if ($this->current_page + $i <= $this->count_pages) {
        $pages_right .= '<li class="page-item">
                    <a class="page-link" href="' . $this->getLink($this->current_page + $i) . '">
                    ' . ($this->current_page + $i) . '
                    </a>
              </li>';
    ...
    return '<nav aria-label="Пример навигации по страницам">
            <ul class="pagination">' . $start_page . $back . $pages_left .
      '<li class="page-item active">
                    <a class="page-link" href="#">' . $this->current_page . '</a>
                </li>' .
